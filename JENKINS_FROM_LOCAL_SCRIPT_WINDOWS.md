...
if [[ -v LOCAL_REPO_ROOT ]]; then
    echo "LOCAL_REPO_ROOT is set (exists) to $LOCAL_REPO_ROOT."
else
    echo "LOCAL_REPO_ROOT is not set (does not exist)."
    exit 3
fi
...
mkdir $BUILD_NUMBER
cd $BUILD_NUMBER
git clone $LOCAL_REPO_ROOTslink_group.ts.adligo.org
